...
class LoginDialog(BaseDialog):
    ...
    def create_widgets(self):
        ...
        ttk.Label(content, text=g_("Username:")).grid(
            row=1, column=0, sticky=tk.E, padx=PAD, pady=PAD)
        self.username_entry = ttk.Entry(content, textvariable=self.username)
        self.username_entry.bind("<Return>", self.on_enter_key_pressed)
        self.username_entry.grid(row=1, column=1, sticky=tk.W+tk.E, ipadx=PAD,
                                 ipady=PAD, padx=PAD, pady=PAD)
        ttk.Label(content, text=g_("Password:")).grid(
            row=2, column=0, sticky=tk.E, padx=PAD, pady=PAD)
        ...
        self.password_entry.grid(row=2, column=1, sticky=tk.W+tk.E, ipadx=PAD,
                                 ipady=PAD, padx=PAD, pady=PAD)
        content.pack(expand=1, fill=tk.BOTH)
        self.username.trace("w", self.on_username_or_password_changed)
        self.password.trace("w", self.on_username_or_password_changed)
        self.on_username_or_password_changed()

    def on_username_or_password_changed(self, *_):
